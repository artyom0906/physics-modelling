...
public class Controller implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        points.add(new Point(new Vec2D(0, 0), new Vec2D(1, 1)));
        points.add(new Point(new Vec2D(200, 200), new Vec2D(-1, -1)));
        points.add(new Point(new Vec2D(200, 100), new Vec2D(-1, 0)));
        points.add(new Point(new Vec2D(400, 100), new Vec2D(1, 0)));
        Random r = new Random();
        double t = r.nextFloat() * 2 * Math.PI - Math.PI;
        float vx = 0.5f;
        float vy = 0.5f;
        /*points.add(new Point(new Vec2D(r.nextFloat() * 600, r.nextFloat()*400), new Vec2D((float) (vx * Math.sin(t)), (float) (vy * Math.cos(t)))));
        for (int i = 0; i < 100; i++) {
